...
// ============================================================================
// REGENERATE FINANCIAL STATEMENT ROUTE
// ============================================================================

export const regenerateStatementRequestSchema = z.object({
  statementCode: z.string().min(1, "Statement code is required"),
  reportingPeriodId: z.number().int().positive("Reporting period ID is required"),
  projectType: z.string().min(1, "Project type is required"),
  facilityId: z.number().int().positive("Facility ID is required"),
  deleteExisting: z.boolean().default(true).describe("Whether to delete existing report before regenerating"),
});

export const regenerateStatementResponseSchema = z.object({
  success: z.boolean(),
  message: z.string(),
  deletedReportId: z.number().optional(),
  deletedWorkflowLogs: z.number().optional(),
  executionDataFound: z.boolean(),
...
export const regenerateStatement = createRoute({
  path: "/financial-reports/regenerate",
  method: "post",
  tags,
  summary: "Regenerate financial statement from current execution data",
  description: "Deletes existing financial report (if any) and regenerates the statement from current execution data. Use this to fix reports that became empty after execution data was deleted and re-entered.",
  request: {
    body: jsonContent(
      regenerateStatementRequestSchema,
      "Regeneration parameters"
    ),
  },
  responses: {
    [HttpStatusCodes.OK]: jsonContent(
      regenerateStatementResponseSchema,
      "Financial statement regenerated successfully"
    ),
    [HttpStatusCodes.BAD_REQUEST]: jsonContent(
      z.object({
        message: z.string(),
...
export type RegenerateStatementRoute = typeof regenerateStatement;