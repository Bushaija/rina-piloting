...
/**
 * Handler for regenerating financial statements from current execution data
 * 
 * This endpoint:
 * 1. Deletes existing financial report for the given parameters (if any)
 * 2. Checks if execution data exists for the parameters
 * 3. Returns status so user can then call generateStatement to view fresh data
 * 
 * Use this to fix reports that became empty after execution data was deleted and re-entered.
 */
export const regenerateStatement: AppRouteHandler<RegenerateStatementRoute> = async (c) => {
  try {
    // Get user context
    const userContext = await getUserContext(c);
    const accessibleFacilityIds = userContext.accessibleFacilityIds;
    ...
    let deletedReportId: number | undefined;
    let deletedWorkflowLogs = 0;

    // Step 1: Delete existing report if requested
    if (deleteExisting) {
      // Find all reports matching the criteria (there might be multiple)
      const existingReports = await db.query.financialReports.findMany({
        where: and(
      ...
      for (const existingReport of existingReports) {
        // Delete workflow logs first (foreign key constraint)
        const deletedLogs = await db.delete(financialReportWorkflowLogs)
          .where(eq(financialReportWorkflowLogs.reportId, existingReport.id))
          .returning();
        deletedWorkflowLogs += deletedLogs.length;

        // Delete the report
        await db.delete(financialReports)
          .where(eq(financialReports.id, existingReport.id));

        deletedReportId = existingReport.id;
        console.log(`[regenerateStatement] Deleted report ID: ${existingReport.id} with ${deletedLogs.length} workflow logs`);
      }
    }

    // Step 2: Check if execution data exists
    const dataEngine = new DataAggregationEngine(db);
    ...
    // Collect event data to check if any exists
    const eventData = await dataEngine.collectEventData(dataFilters, eventCodes);
    const executionDataFound = eventData.currentPeriod.length > 0;
    const totalEvents = eventData.currentPeriod.length + eventData.previousPeriod.length;

    // Get facility info for response
    const facility = await db.query.facilities.findFirst({
      where: eq(facilities.id, facilityId),
      with: { district: true }
    });
    ...
    return c.json({
      success: true,
      message: deletedReportId 
        ? `Deleted old report #${deletedReportId}${deletedWorkflowLogs > 0 ? ` and ${deletedWorkflowLogs} workflow logs` : ''}. ${executionDataFound ? 'Execution data found - call generateStatement to view fresh data.' : 'No execution data found for these parameters.'}`
        : `No existing report found to delete. ${executionDataFound ? 'Execution data found - call generateStatement to view fresh data.' : 'No execution data found for these parameters.'}`,
      deletedReportId,
      deletedWorkflowLogs,
      executionDataFound,
      ...
      context: {
        ...
        district: facility?.district?.name || 'Unknown',
        reportingPeriodId,
        year: reportingPeriod?.year,
      },
      nextStep: executionDataFound 
        ? 'Call POST /financial-reports/generate-statement with the same parameters to view the regenerated statement'
        : 'Add execution data first, then call generateStatement'
    }, HttpStatusCodes.OK);

  } catch (error: any) {
    console.error('[regenerateStatement] Error:', error);
    ...
    return c.json(
      { message: "Failed to regenerate statement", error: error.message },
      HttpStatusCodes.INTERNAL_SERVER_ERROR
    );
  }
};
