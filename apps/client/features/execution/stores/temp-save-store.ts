...
export interface TempSaveMetadata {
  projectId?: number | null  // Project ID for better scoping
  ...
  district?: string
  facilityType: string
  quarter?: string // Quarter to differentiate each unique facility's execution
}
...
// Enhanced to include all critical context: projectId, program, facility, facility type, facility name, reporting period, quarter, and mode
...
export const generateSaveId = (metadata: TempSaveMetadata): string => {
  const parts = [
    ...
    metadata.facilityType,          // Facility type (hospital, health_center)
    metadata.facilityId,            // Facility ID
    metadata.facilityName,          // Facility name (for human readability and extra uniqueness)
    metadata.reportingPeriod,       // Reporting period
    metadata.quarter || 'all',      // Quarter (Q1, Q2, Q3, Q4)
