...
export function EnhancedExecutionFormAutoLoad({
  ...
  // Enhanced to include all key identifiers for unique facility execution differentiation
  const draftMeta = useMemo(() => {
    ...
    const qpProjectId = projectIdProp || Number(searchParams?.get("projectId") || 0) || 0;
    ...
    return {
      ...
      quarter: quarter, // Quarter is critical for differentiating each facility's execution
    };
  }, [searchParams, facilityType, projectType, effectiveMode, currentReportingPeriod?.id, facilityIdProp, facilityNameProp, programNameProp, reportingPeriodIdProp, projectIdProp, quarter]);

  // Generate a stable draft ID - use consistent identifiers from URL params
  // Format: exec_{projectId}_{facilityId}_{facilityType}_{program}_{reportingPeriodId}_{quarter}
  // This ensures each unique facility's execution is differentiated by all key parameters
  const draftId = useMemo(() => {
    ...
    // Include all key identifiers: projectId, facilityId, facilityType, program, reportingPeriod, quarter
    const raw = `exec_${projectIdValue}_${fId}_${fType}_${program}_${rpId}_${quarter}`;
    const id = raw.replace(/\s+/g, '_').toLowerCase();
    
    console.log('[Draft] Generated draftId:', id, { 
    ...
    return id;
  }, [searchParams, facilityType, quarter, projectType, currentReportingPeriod?.id, facilityIdProp, reportingPeriodIdProp, programNameProp, projectIdProp]);
  ...
  const removeTemporary = useTempSaveStore(s => s.removeTemporary);
  const lastSavedIso = useTempSaveStore(s => s.saves[draftId]?.timestamps.lastSaved);
  const hasDraft = useTempSaveStore(s => !!s.saves[draftId]);
  ...
  const handleSmartSubmission = useCallback(async () => {
    ...
    try {
      ...
      // This ensures the draft is removed immediately when the execution is submitted
